...
// Deterministic scoring algorithm (no randomness)
function normalizeSkill(s: string): string {
  return s.toLowerCase().replace(/[^a-z0-9+#]/g, "");
}

function scoreResume(parsed: any, role: any, resumeText: string) {
  ...
  if (matchedRequired.length >= role.required_skills.length * 0.7) strengths.push("Strong skill match for the role");
  ...
  if (projects.length >= 3) strengths.push("Diverse project portfolio");
  if (metrics.length >= 3) strengths.push("Good use of quantified impact metrics");
  if ((parsed.certifications || []).length > 0) strengths.push("Relevant certifications");
  if (strengths.length === 0) strengths.push("Resume has foundational content to build upon");

  // Recommendations
  const recommendations: string[] = [];
  if (missingSkills.length > 0) recommendations.push(`Add missing skills: ${missingSkills.slice(0, 5).join(", ")}`);
  if (metrics.length < 3) recommendations.push("Add more quantified achievements");
  if (projects.length < 2) recommendations.push("Include more relevant projects");
  if (parsed.total_years_experience < role.minimum_years) recommendations.push(`Role requires ${role.minimum_years}+ years of experience`);
  if ((parsed.certifications || []).length === 0) recommendations.push("Add relevant certifications");
  if (clampedOverall < 70) recommendations.push("Use stronger action verbs and quantify accomplishments");
...
// Job roles lookup (embedded for edge function)
...
serve(async (req) => {
  ...
  try {
    ...
    const result = scoreResume(parsed, role, resumeText);
    ...
    return new Response(JSON.stringify({ ...result, analysisId: analysis.id }), {
